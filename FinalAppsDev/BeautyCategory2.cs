...
namespace finalAppsDevProject
{
    public partial class BeautyCategory2 : Form
    {
        ...
        private void Calculate_allcost_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrWhiteSpace(_beautyCategory.MyProductName) ||
                string.IsNullOrWhiteSpace(_beautyCategory.TotalRawMaterialCost) ||
                string.IsNullOrWhiteSpace(_beautyCategory.UtilitiesCost) ||
                string.IsNullOrWhiteSpace(_beautyCategory.TotalLaborCost) ||
                string.IsNullOrWhiteSpace(_beautyCategory.LaborCostPerUnit) ||
            ...
            {
                // Parse input values
                decimal rmc = decimal.Parse(_beautyCategory.TotalRawMaterialCost);
                decimal uc = decimal.Parse(_beautyCategory.UtilitiesCost);
                decimal tlc = decimal.Parse(_beautyCategory.TotalLaborCost);
                decimal laborPerUnit = decimal.Parse(_beautyCategory.LaborCostPerUnit);
                ...
                string productName = _beautyCategory.MyProductName;
        ...
        {
            ProductSummaryDialog1 summaryForm = new ProductSummaryDialog1(this);
            summaryForm.SetSummaryData(productName, servings, tpc, srpTotal, srpPerUnit);
            summaryForm.ShowDialog();
        }

        private void BeautyCategory2_Load(object sender, EventArgs e)
        {
            bool isChecked = Btchx_Chckbox.Checked;
